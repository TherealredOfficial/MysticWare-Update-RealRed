-- MysticWare | discord.gg/LunarRBX
...
warningNotification("MysticWare", "Loaded Succesfully! Made By TheRealRed, Enjoy!", 5)
...
runFunction(function()
	...
	local DisablerTable = {
		Values = {
			...
			MainSpeed = DisablerSpeedVal.Value * 10000 + 9.999
		},
		Messages = {
			Title = "Disabler",
	...
	Disabler = GuiLibrary.ObjectsThatCanBeSaved.LunarWindow.Api.CreateOptionsButton({
        Name = "Disabler",
...
local function MysticWare(func) func() end

getgenv().TexturePack = true -- enables the TexturePack
getgenv().DamageIndicators = true -- enables the DamageIndicators

MysticWare(function()
    local texturepack = {Enabled = false}
	local packDropdown = {Value = "Lunar New"}
	local ogpackloader = game:GetObjects("rbxassetid://14027120450")
	local ogtxtpack = ogpackloader[1]
	ogtxtpack.Name = "Lunar Old"
	...
	local function refreshViewmodel(child)
		for i,v in pairs(textures[packDropdown.Value]:GetChildren()) do
			if string.lower(v.Name) == child.Name and child.Parent.Name ~= child.Name then
				...
				vmmodel.CFrame = vmmodel.CFrame * (packDropdown.Value == "Lunar Old" and CFrame.new(0, -0.2, 0) or packDropdown.Value == "Lunar New" and CFrame.new(0.2, -0.2, 0) or packDropdown.Value == "4zze" and CFrame.new(0.8,0.1,0.7)) * CFrame.Angles(math.rad(90),math.rad(-130),math.rad(0))
				...
				if (string.lower(child.Name):find("axe")) and not (string.lower(child.Name):find("pickaxe")) then vmmodel.CFrame = vmmodel.CFrame * CFrame.Angles(math.rad(-55),math.rad(-30),math.rad(50)) * (packDropdown.Value == "Lunar New" and CFrame.new(-0.2,0,0.2) or packDropdown.Value == "4zze" and CFrame.new(-1.5,0,-0.8)) end
				...
				charmodel.CFrame = charmodel.CFrame * (packDropdown.Value == "Lunar Old" and CFrame.new(0, -0.5, 0) or packDropdown.Value == "Lunar New" and CFrame.new(0.2, -0.9, 0) or packDropdown.Value == "4zze" and CFrame.new(0.1,-1.2,0)) * CFrame.Angles(math.rad(90),math.rad(-130),math.rad(0))
	...
	texturepack = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
        ...
        Function = function(callback)
            if callback and getgenv().TexturePack then
            ...
            elseif callback and (not getgenv().TexturePack) then
				warningNotification('TexturePack', 'Enable in settings', 5)
...
MysticWare(function()
	...
	local DamageIndicatorCL = {Enabled = true}
	local DamageIndicatorText = {Enabled = true}
    local DamageMSG = {
		'Pop!',
		'Pow!',
		'Hit!',
	...
	local OldIndicator = nil
	DamageIndicator = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
        Name = 'DamageIndicator',
		HoverText = 'Customizes your damage indicator',
        Function = function(callback)
            if callback and getgenv().DamageIndicators then
                OldIndicator = debug.getupvalue(
                ...
                debug.setupvalue(
					...
					10, {
						Create = function(self, obj, ...)
							task.spawn(function()
								pcall(function()
									if DamageIndicatorCL.Enabled then
										obj.Parent.TextColor3 = Color3.fromHSV(
									...
									if DamageIndicatorText.Enabled then
										obj.Parent.Text = DamageMSG[math.random(
											1, #DamageMSG
										)]
									end
								end)
			...
			elseif callback and (not getgenv().DamageIndicators) then
				warningNotification('DamageIndicators', 'Enable in settings', 5)
				DamageIndicators:ToggleButton(false)
				return
			elseif not callback then
				debug.setupvalue(
					bedwars.DamageIndicator, 
	...
	DamageIndicatorColor = DamageIndicator.CreateColorSlider({
		Name = 'Color',
		HoverText = 'Color of the indicators',
		Function = function() end
	})
	DamageIndicatorCL = DamageIndicator.CreateToggle({
		Name = 'Color',
		Default = true,
		HoverText = 'Changes the indicator color',
		Function = function() end
	})
	DamageIndicatorText = DamageIndicator.CreateToggle({
		Name = 'Text',
		Default = true,
		HoverText = 'Changes the indicator text',
		Function = function() end
	})
end)
